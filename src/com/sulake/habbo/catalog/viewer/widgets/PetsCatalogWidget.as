package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_422 >= var_308.length)
         {
            return "";
         }
         var petColor:IPetColor = var_308[var_422] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_604[var_1560] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_422 = param1.index;
         if(var_422 < 0 || var_422 > var_308.length)
         {
            var_422 = 0;
         }
         updateImage();
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_141,onTextWindowEvent);
         var_548 = new Map();
         ...
         var_892 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_308 = [];
         var_604 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_892);
               if(_loc11_)
               {
                  var_604 = _loc11_.breeds;
                  var_308 = _loc11_.colors;
                  if(var_308 == null)
                  {
                     var_308 = [];
                  }
         ...
         while(_loc6_ < var_604.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_892,_loc12_.id),getRaceLocalizationKey(var_892,_loc12_.id)));
            _loc6_++;
         }
         var_548.add(var_892,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_887,onApproveNameResult);
      ...
      private function updateImage() : void
      {
         ...
         if(var_422 >= 0 && var_422 < var_308.length)
         {
            _loc7_ = var_308[var_422] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_604[var_1560] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_892,_loc3_.id,_loc2_,AvatarScaleType.const_51);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_332,AvatarAction.const_683);
