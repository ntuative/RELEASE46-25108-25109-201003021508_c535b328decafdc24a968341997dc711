package com.sulake.habbo.inventory.achievements
{
   ...
   public class AchievementsModel implements IInventoryModel
   {
      ...
      public function AchievementsModel(param1:HabboInventory, param2:IHabboWindowManager, param3:IHabboCommunicationManager, param4:IAssetLibrary, param5:IHabboLocalizationManager, param6:ISessionDataManager)
      {
         ...
         var_352 = param6;
         var_352.events.addEventListener(BadgeImageReadyEvent.const_119,onBadgeImageReady);
         var_349 = new Array();
      ...
      public function addAchievement(param1:Achievement, param2:Boolean = true) : void
      {
         ...
         var _loc4_:int = 0;
         while(_loc4_ < var_349.length)
         {
            _loc5_ = var_349[_loc4_] as Achievement;
            if(_loc5_.type == param1.type && _loc5_.level < param1.level)
            {
               _loc5_.level = param1.level;
               _loc5_.badgeId = param1.badgeId;
               _loc5_.image = var_352.getBadgeImage(param1.badgeId);
               _loc3_ = true;
            }
            _loc4_++;
         }
         if(!_loc3_)
         {
            param1.image = var_352.getBadgeImage(param1.badgeId);
            var_349.push(param1);
         }
         if(param2)
         {
      ...
      public function removeAchievement(param1:String, param2:int) : void
      {
         ...
         _loc4_ = var_349[0] as Achievement;
      ...
      {
         if(param1 == InventoryCategory.const_317 && false)
         {
            var_11.events.dispatchEvent(new Event(HabboInventoryTrackingEvent.HABBO_INVENTORY_TRACKING_EVENT_ACHIEVEMENTS));
         }
      }
      ...
      public function getAchievements() : Array
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < var_349.length)
         {
            _loc3_ = var_349[_loc2_] as Achievement;
            _loc1_.push(_loc3_);
            _loc2_++;
         }
         return _loc1_;
